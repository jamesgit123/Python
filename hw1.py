...
def onelayer(X, Y, layersize=10):
    ...
    tf.cast(batch_xentropy,tf.float32)
    batch_loss = tf.reduce_mean(batch_xentropy)
    tf.cast(batch_loss,tf.float32)
...
def twolayer(X, Y, hiddensize=30, outputsize=10):
    ...
    w1 = tf.Variable(tf.zeros([784, 30]),tf.float32)
    b1 = tf.Variable(tf.truncated_normal(([30]),stddev = 0.1),tf.float32)
    preds2 = tf.nn.relu(tf.matmul(X, w1) + b1)
    tf.cast(preds2,tf.float32)
    w2 = tf.Variable(tf.zeros([30, 10]),tf.float32)
    b2 = tf.Variable(tf.zeros([10]),tf.float32)

    logits = tf.matmul(preds2, w2) + b2
    tf.cast(logits,tf.float32)
    preds = tf.nn.softmax(logits)
    tf.cast(preds,tf.float32)
...
def convnet(X, Y, convlayer_sizes=[10, 10], \
    ...
    w1 = tf.Variable(tf.truncated_normal(([3,3,1,10]), stddev=0.1),tf.float32)
    b1 = tf.Variable(tf.constant(0.1, shape=([10])),tf.float32)
    conv1 = tf.nn.relu(tf.nn.conv2d(X, w1, strides=[1, 1, 1, 1], padding='SAME') + b1)

    w2 = tf.Variable(tf.truncated_normal(([3,3,10,10]), stddev=0.1),tf.float32)
    b2 = tf.Variable(tf.constant(0.1, shape=([10])),tf.float32)
    conv2 = tf.nn.relu(tf.nn.conv2d(conv1, w2, strides=[1, 1, 1, 1], padding='SAME') + b2)

    
    w = tf.Variable(tf.zeros([7840, 10]),tf.float32)

    b = tf.Variable(tf.zeros([10]),tf.float32)

    logits = tf.matmul(tf.reshape(conv2, [-1, 7840]),w) + b
